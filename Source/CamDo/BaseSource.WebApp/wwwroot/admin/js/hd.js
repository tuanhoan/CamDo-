...
function getHistoryLoanExtra(hopDongId) {
    $.ajax({
        type: "GET",
        ...
        success: function (data) {
            ...
            $('#lblTongTienVayHienTai').text(tongTienVayHienTai + " VND");
            traBotGoc();
        }
    })
}
function traBotGoc() {
    $('#frmTraBotGoc').on("submit", function (e) {
        e.preventDefault();
        var $form = $(this);
        var $btnSubmit = $form.find("button[type='submit']");
        $btnSubmit.attr("disabled", "true");
        var hopDongId = $('#frmTraBotGoc #HopDongId').val();
        $.ajax({
            method: $form.attr("method"),
            url: $form.attr("action"),
            data: $form.serializeArray(),
            ...
            success: function (res) {
                ...
                $form.find(".field-validation-valid").empty();
                if (res.isSuccessed == true) {
                    getHistoryLoanExtra(hopDongId);
                    $('.lblTongTienVayHienTai').text(format(res.resultObj) + " VND");
                } else if (res.validationErrors != null && res.validationErrors.length) {
                    $.each(res.validationErrors, function (i, v) {
                        console.log(v);
                        $form.find("span[data-valmsg-for='" + v.pos + "']").html(v.error);
